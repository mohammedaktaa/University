...
<table class="table table-responsive table-condensed table-striped hidden-xs">
  ...
  <tbody>
    ...
    <tr style="color: #0a001f">
      <td>
        <i class="fa {{ $item->icon }}"></i>
        <a class="{{ $item->is_file ? 'file' : 'folder'}}-item clickable" data-id="{{ $item->is_file ? $item->url : $item->path }}" style="color: #0a001f">
      ...
      <td>
        ...
        <a href="javascript:trash('{{ $item->name }}')">
          <i class="fa fa-trash fa-fw"style="color:#ff3a44 !important; "></i>
        </a>
        @if($item->thumb)
        <a href="javascript:cropImage('{{ $item->name }}')">
          <i class="fa fa-crop fa-fw"style="color: #ff3a44 !important;" ></i>
        </a>
        <a href="javascript:resizeImage('{{ $item->name }}')">
          <i class="fa fa-arrows fa-fw"style="color: #ff3a44 !important;"></i>
        </a>
        @endif
        @endif
      </td>
